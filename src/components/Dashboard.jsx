import React from "react";
import "./Dashboard.css";

function Dashboard() {
  return (
    <div>
      <div className="top">
        <div className="acct-actions">
          <div className="actions">
            <a href="#" className="li logout-link">
              <i class="fa fa-sign-out" aria-hidden="true"></i>Log out
            </a>
          </div>
        </div>
      </div>
      {/* Body of the page */}
      <div className="main">
        <div className="statuses full">
          <div className="status">
            <div className="status-ride">
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">pst_iren</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://scontent-yyz1-1.cdninstagram.com/v/t51.2885-15/sh0.08/e35/s640x640/62023616_638623226635197_7956289733882868130_n.jpg?_nc_ht=scontent-yyz1-1.cdninstagram.com&_nc_cat=109&_nc_ohc=X0PmOkc_Dm4AX_ctPGl&oh=d98528a987ce5e7a20dd3c7f82ee73f4&oe=5EC9B663"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">simpl...</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">fagbs_id</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://mir-s3-cdn-cf.behance.net/user/276/a217775903389.550d7721899d8.jpg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">adekun...</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://is5-ssl.mzstatic.com/image/thumb/Music113/v4/74/f2/c2/74f2c2b7-842d-3af6-1ba2-d4563b897c8f/pr_source.png/800x800bb.jpeg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">iamjode..</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://dailypost.ng/wp-content/uploads/2019/06/Biodun-fatoyinbo.jpg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">biodu...</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">jjhairs...</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://is5-ssl.mzstatic.com/image/thumb/Music113/v4/74/f2/c2/74f2c2b7-842d-3af6-1ba2-d4563b897c8f/pr_source.png/800x800bb.jpeg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">iamjode..</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="https://dailypost.ng/wp-content/uploads/2019/06/Biodun-fatoyinbo.jpg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">biodu...</a>
                </div>
              </div>
              <div className="status_">
                <div className="status-pix">
                  <a href="#">
                    <img
                      className="pix"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-link">
                  <a href="#">jjhairs...</a>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="section">
          <div className="top-section">
            <div className="section-title">
              <div className="profile-pic">
                <a href="#">
                  <img
                    className="picture"
                    src="data:image/jpeg;base64,"
                    alt="user"
                  />
                </a>
              </div>
              <div className="profile">
                <a href="#" className="profile_tag">
                  pst_iren
                </a>
              </div>
            </div>
            <div className="opt">
              <a href="#" className="options">
                <div className="op" />
                <div className="op" />
                <div className="op" />
              </a>
            </div>
          </div>
          <div className="upload">
            <img
              className="upload-content"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTBXEZnYDFnSS5mX8i9LCJAh0iAvaztg9wPKonWMlOi_yn7Hs2w"
            />
          </div>
          <div className="upload-actions">
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://cdn0.iconfinder.com/data/icons/instagram-ui-1/24/Instagram-UI_comment-512.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://qph.fs.quoracdn.net/main-qimg-094b6418c35690a0a9425642728f081b"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li aside-tag">
              <a href="#">
                <img
                  className="tag"
                  height="28px"
                  src="https://cdn.clipart.email/51d663379c2127f9240f6b677bce03a2_flag-instagram-interface-save-tag-icon_512-512.png"
                  alt
                />
              </a>
            </button>
          </div>
          <div className="liked">
            <p className="liked-count">
              Liked by{" "}
              <a className="likes" href="#">
                olife_onyinye
              </a>{" "}
              and <button className="others">others</button>
            </p>
          </div>
          <div className="upload-caption">
            <span className="post">
              <a href="#" className="likes">
                pst_iren
              </a>
              <span className="post-link">
                Yesterday in pictures. Great services in ABUJA and LAGOS. What a
                month this has been.
              </span>
            </span>
          </div>
          <div className="upload-caption comments liked">
            <a className="likes comment" href="#">
              View all <span>58</span> comments
            </a>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                justified_kay
              </a>
              <span className="post-link">
                Yesterday was awesome! It's a year of movement!
              </span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push2">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                _ginnie
              </a>
              <span className="post-link">It was a great experience.</span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="time-stamp liked">18 HOURS AGO</div>
          <div className="submit">
            <div className>
              <form className="ssub" method="POST">
                <input
                  type="text"
                  placeholder="Add a comment…"
                  className="texta"
                />
                <button className="button u-actions-li">Post</button>
              </form>
            </div>
          </div>
        </div>
        <div className="section section2">
          <div className="top-section">
            <div className="section-title">
              <div className="profile-pic">
                <a href="#">
                  <img
                    className="picture"
                    src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSB-uKOK8M_Goa1J6xoOlFkKbSXGf6c0EMXzuvfAvQOqHXGI-F1"
                    alt="user"
                  />
                </a>
              </div>
              <div className="profile">
                <a href="#" className="profile_tag">
                  babsadenuga
                </a>
              </div>
            </div>
            <div className="opt">
              <a href="#" className="options">
                <div className="op" />
                <div className="op" />
                <div className="op" />
              </a>
            </div>
          </div>
          <div className="upload">
            <img
              className="upload-content"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcSB-uKOK8M_Goa1J6xoOlFkKbSXGf6c0EMXzuvfAvQOqHXGI-F1"
            />
          </div>
          <div className="upload-actions">
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://cdn0.iconfinder.com/data/icons/instagram-ui-1/24/Instagram-UI_comment-512.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://qph.fs.quoracdn.net/main-qimg-094b6418c35690a0a9425642728f081b"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li aside-tag">
              <a href="#">
                <img
                  className="tag"
                  height="28px"
                  src="https://cdn.clipart.email/51d663379c2127f9240f6b677bce03a2_flag-instagram-interface-save-tag-icon_512-512.png"
                  alt
                />
              </a>
            </button>
          </div>
          <div className="liked">
            <p className="liked-count">
              Liked by{" "}
              <a className="likes" href="#">
                akolade_olusola
              </a>{" "}
              and <button className="others">others</button>
            </p>
          </div>
          <div className="upload-caption">
            <span className="post">
              <a href="#" className="likes">
                babsadenuga
              </a>
              <span className="post-link">
                My name is Babatunde Adenuga and I am here to tell stories. To
                inspire stories. To lead powerful stories and to build stories
                that inspire hope and faith in the greatness of Africa.
              </span>
            </span>
          </div>
          <div className="upload-caption comments liked">
            <a className="likes comment" href="#">
              View all <span>35</span> comments
            </a>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                akolade_olusola
              </a>
              <span className="post-link">
                Grandezo!
                {/* You're an inspiration to me, I love you very much! */}
              </span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push2">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                kenny_great
              </a>
              <span className="post-link">The sun is the limit bro!</span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push push2">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="time-stamp  liked">12 HOURS AGO</div>
          <div className="submit">
            <div className>
              <form className="ssub" method="POST">
                <input
                  type="text"
                  placeholder="Add a comment…"
                  className="texta"
                />
                <button className="button u-actions-li">Post</button>
              </form>
            </div>
          </div>
        </div>
        <div className="section section2">
          <div className="top-section">
            <div className="section-title">
              <div className="profile-pic">
                <a href="#">
                  <img
                    className="picture"
                    src="data:image/jpeg;base64,"
                    alt="user"
                  />
                </a>
              </div>
              <div className="profile">
                <a href="#" className="profile_tag">
                  thesamuelbello
                </a>
              </div>
            </div>
            <div className="opt">
              <a href="#" className="options">
                <div className="op" />
                <div className="op" />
                <div className="op" />
              </a>
            </div>
          </div>
          <div className="upload">
            <img
              className="upload-content"
              src="https://pbs.twimg.com/profile_images/1230880902932303874/KHbTvA0N_400x400.jpg"
            />
          </div>
          <div className="upload-actions">
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://cdn0.iconfinder.com/data/icons/instagram-ui-1/24/Instagram-UI_comment-512.png"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li">
              <a href="#">
                <img
                  height="28px"
                  src="https://qph.fs.quoracdn.net/main-qimg-094b6418c35690a0a9425642728f081b"
                  alt
                />
              </a>
            </button>
            <button className="u-actions-li aside-tag">
              <a href="#">
                <img
                  className="tag"
                  height="28px"
                  src="https://cdn.clipart.email/51d663379c2127f9240f6b677bce03a2_flag-instagram-interface-save-tag-icon_512-512.png"
                  alt
                />
              </a>
            </button>
          </div>
          <div className="liked">
            <p className="liked-count">
              Liked by{" "}
              <a className="likes" href="#">
                akolade_olusola
              </a>{" "}
              and <button className="others">others</button>
            </p>
          </div>
          <div className="upload-caption">
            <span className="post">
              <a href="#" className="likes">
                thesamuelbello
              </a>
              <span className="post-link">Absolutely flawless!</span>
            </span>
          </div>
          <div className="upload-caption comments liked">
            <a className="likes comment" href="#">
              View all <span>53</span> comments
            </a>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                akolade_olusola
              </a>
              <span className="post-link">El Patron...</span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push2">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="upload-caption comments">
            <span className="post">
              <a href="#" className="likes">
                the_kendrick
              </a>
              <span className="post-link">Boss boss...</span>
              <span className="post-link">
                <button className="u-actions-li  like">
                  <a href="#" className="push">
                    <img
                      height="12px"
                      src="https://www.pinclipart.com/picdir/big/33-337369_heart-shaped-clipart-instagram-heart-sign-icon-transparent.png"
                      alt
                    />
                  </a>
                </button>
              </span>
            </span>
          </div>
          <div className="time-stamp liked">22 HOURS AGO</div>
          <div className="submit">
            <div className>
              <form className="ssub" method="POST">
                <input
                  type="text"
                  placeholder="Add a comment…"
                  className="texta"
                />
                <button className="button u-actions-li">Post</button>
              </form>
            </div>
          </div>
        </div>
        {/* Aside section- Pc screen alone */}
        <div className="aside-section">
          <div className="aside1">
            <div className="user-profile">
              <div className="own-profile-pic">
                <a href="#">
                  <img
                    className="own-picture"
                    src="https://pbs.twimg.com/profile_images/1017823687804882944/YMzUeY9d_400x400.jpg"
                    alt="user"
                  />
                </a>
              </div>
              <div className="user">
                <a href="#" className="own-account">
                  <span className="identity">akolade_olusola</span>
                  <span className="username">Akolade</span>
                </a>
              </div>
            </div>
          </div>
          <div className="aside2">
            <div className="aside2-top">
              <div className="aside2-top1">Messages</div>
              <div className="aside2-top2">
                <a href="#">Watch All</a>
              </div>
            </div>
            <div className="aside2-body">
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">Faheema</a>
                  </span>
                  <span className="time-stamp">12.04 pm</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="https://scontent-yyz1-1.cdninstagram.com/v/t51.2885-15/sh0.08/e35/s640x640/62023616_638623226635197_7956289733882868130_n.jpg?_nc_ht=scontent-yyz1-1.cdninstagram.com&_nc_cat=109&_nc_ohc=X0PmOkc_Dm4AX_ctPGl&oh=d98528a987ce5e7a20dd3c7f82ee73f4&oe=5EC9B663"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">Jezna</a>
                  </span>
                  <span className="time-stamp">2 HOURS AGO</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">Hiba</a>
                  </span>
                  <span className="time-stamp">15 HOURS AGO</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="https://mir-s3-cdn-cf.behance.net/user/276/a217775903389.550d7721899d8.jpg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">Sherin</a>
                  </span>
                  <span className="time-stamp">3 HOURS AGO</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="https://is5-ssl.mzstatic.com/image/thumb/Music113/v4/74/f2/c2/74f2c2b7-842d-3af6-1ba2-d4563b897c8f/pr_source.png/800x800bb.jpeg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">Rizwa</a>
                  </span>
                  <span className="time-stamp">7 HOURS AGO</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="https://dailypost.ng/wp-content/uploads/2019/06/Biodun-fatoyinbo.jpg"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">biodunfatoyinbo</a>
                  </span>
                  <span className="time-stamp">23 HOURS AGO</span>
                </div>
              </div>
              <div className="story">
                <div className="profile-pic">
                  <a href="#">
                    <img
                      className="picture"
                      src="data:image/jpeg;base64,"
                      alt="user"
                    />
                  </a>
                </div>
                <div className="profile-info">
                  <span className="profile-link">
                    <a href="#">jjhairston</a>
                  </span>
                  <span className="time-stamp">2 HOURS AGO</span>
                </div>
              </div>
            </div>
          </div>
          <div className="aside3">
            <div className="aside3-t">
              <div className="aside2-top">
                <div className="aside2-top1 tp3">Likes and Matches</div>
                <div className="aside2-top2">
                  <a href="#">See All</a>
                </div>
              </div>
              <div className="aside3-body">
                <div className="st">
                  <div className="suggestions">
                    <div className="sugg-pix">
                      <a href="#">
                        <img
                          className="picture"
                          src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1024px-User_icon_2.svg.png"
                          alt="user"
                        />
                      </a>
                    </div>
                    <div className="profile-info">
                      <span className="profile-link s-link">
                        <a href="#">Faheema</a> <br />
                        <span className="time-stamp">Likes you 3 min ago</span>
                      </span> 
                    </div>
                    <button className="follow">
                      <span href="#" className="ff">
                      12.04 pm
                      </span>
                    </button>
                  </div>
                  <div className="suggestions">
                    <div className="sugg-pix">
                      <a href="#">
                        <img
                          className="picture"
                          src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1024px-User_icon_2.svg.png"
                          alt="user"
                        />
                      </a>
                    </div>
                    <div className="profile-info">
                      <span className="profile-link s-link">
                        <a href="#">Jezna</a>
                        <br />
                        <span className="time-stamp">Likes you 5 min ago</span>
                      </span>
          
                    </div>
                    <button className="follow">
                      <a href="#" className="ff">
                      12.04 pm
                      </a>
                    </button>
                  </div>
                  <div className="suggestions">
                    <div className="sugg-pix">
                      <a href="#">
                        <img
                          className="picture"
                          src="https://upload.wikimedia.org/wikipedia/commons/thumb/1/12/User_icon_2.svg/1024px-User_icon_2.svg.png"
                          alt="user"
                        />
                      </a>
                    </div>
                    <div className="profile-info">
                      <span className="profile-link s-link">
                        <a href="#">Hiba</a>
                        <br />
                        <span className="time-stamp">Likes you 1h ago</span>
                      </span>
                    
                    </div>
                    <button className="follow">
                      <a href="#" className="ff">
                      12.04 pm
                      </a>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Dashboard;
